...
class GlobalSearch(BaseSearch):
    ...
    async def asearch(
        ...
        **kwargs: Any,
        ...
        logger.info(f'知识图谱检索和上下文构建耗时: {execution_time:.6f} 秒')
        logger.info(f"QUERY: {query}")
        logger.info(f"HISTORY: {conversation_history.turns}")
        logger.debug(f"context_chunks: {context_chunks}")
        if self.callbacks:
        ...
        logger.debug(f"Reduce 阶段的 LLM 结果: {reduce_response}")
